...
export default (props) => {
  ...
  const shrinkingFractal = useRef({ ...growingFractal.current });
  ...
  const handleResize = () => {
    ...
    setState((state) => ({ ...state, isDrawing: false, isErasing: false }));
    ...
    if (eraseCanvas.current) {
      clearInterval(shrinkingFractal.current.intervalRef);
      shrinkingFractal.current.intervalRef = null;
    ...
    if (drawCanvas.current) {
      clearInterval(growingFractal.current.intervalRef);
      growingFractal.current.intervalRef = null;
  ...
  useEffect(() => {
    const handleScrollY = () => {
      ...
      function animation_loop() {
        ...
        drawCanvasElem.current.style.top = `${
          canvasElemInitialPos.current - canvasElemTotalOffset.current * 0.45
        }px`;
    ...
    return () => {
      if (growingFractal.current.intervalRef)
        clearInterval(growingFractal.current.intervalRef);
  ...
  useEffect(() => {
    if (curColor.current === null) {
      ...
      if (!state.isDrawing) init();
    }
  }, [color]);

  useEffect(() => {
    if (bgOpacity !== undefined) {
      setState((state) => ({ ...state, bgOpacity: bgOpacity }));
    }
  }, [bgOpacity]);
  ...
  useEffect(() => {
    if (state.isDrawing) {
      ...
      growingFractal.current.intervalRef = setInterval(() => {
        onTimer();
      }, 1000 / 64);
    }
  }, [state.isDrawing]);
  ...
  useEffect(() => {
    if (state.isErasing) {
      if (shrinkingFractal.current.intervalRef) {
        clearInterval(shrinkingFractal.current.intervalRef);
      }

      shrinkingFractal.current.intervalRef = setInterval(() => {
        onTimer(true);
      }, 1000 / 64);
    }
  }, [state.isErasing]);
  ...
  function init(isErase) {
    ...
    } else setState((state) => ({ ...state, isErasing: true }));
  ...
  function getGradient(maxR, minR, isErase) {
    ...
    if (!isErase) {
      if (curColor.current == null) {
        ...
        grad.addColorStop(1, `rgba(${sat.r},${sat.g},${sat.b},0.2)`);
      }
    } else {
  ...
  function setPaths() {
    ...
    for (i = 0; i < state.numPaths; i++) {
      maxR =
        state.minMaxRad + Math.random() * (state.maxMaxRad - state.minMaxRad);
      minR = state.minRadFactor * maxR;
      ...
      newCircle.pointList1 = setLinePoints(state.iterations);
      newCircle.pointList2 = setLinePoints(state.iterations);
  ...
  function onTimer(isErase) {
    let drawsPerFrame = state.isForward
      ? state.drawsPerFrame
      : state.drawsPerFrame;
    ...
    for (let j = 0; j < drawsPerFrame; j++) {
      ...
      fractal.paths = draw(
        ...
        fractal.drawCount,
        (paths) => {
          if (fractal.intervalRef === null) return;

          clearInterval(fractal.intervalRef);

          fractal.intervalRef = null;
      ...
      fractal.currentFrameDraw++;
    }

    fractal.currentFrameDraw = 0;

    if (fractal.intervalRef === null) {
      if (!isErase) {
        ...
        setState((state) => ({ ...state, isErasing: false }));
      }
    }
    ...
    for (i = 0; i < paths.length; i++) {
      ...
      if (c.param >= 1) {
        ...
        if (isErase) {
          ...
          c.pointList2 = setLinePoints(state.iterations);
      ...
      _ctx.lineWidth = state.lineWidth;
      ...
      if (c.centerX > drawCanvas.current.width + state.maxMaxRad) {
        endReached = true;
      }
  ...
  return (
    <>
      <div
        className={bgType ?? "fractal"}
        style={{
          opacity: state.bgOpacity,
        }}
      >
        <canvas ref={eraseCanvas} className={"canvas"} id="erase" />
        <canvas ref={drawCanvas} className={"canvas"} id="draw" />
      </div>
      {/* {state.draw && (
        <button
          style={{
          ...
          onClick={() => {
            let copy = [...originalCirclesRef.current];

            for (let i = 0; i < circlesRef.current.drawCount.current; i++) {
              let next = [...copy];
              copy = draw(next, i, undefined, true);
            }

            setState({ ...state, draw: false });
          }}
        >
          <p style={{ color: "black", fontSize: "1em" }}>Redraw</p>
        </button>
      )} */}
    </>
  );
